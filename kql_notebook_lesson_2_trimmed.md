...
| **Multi-attachment messages** | Can dominate all 20 slots | ✓ Ranked by message, not file | ✓ Deduped by (message, filename) |
| **Duplicate attachment rows** | Susceptible | Partially mitigated | ✓ Explicit dedupe |
| **Row count guarantee** | ✗ Duplicates can reduce distinct results | Can exceed 20 | ✓ Exactly 20 |

Version 2 is the right choice when the question is *"which messages are carrying the largest files."* Version 3 is the most reliable for a clean top-20 list — explicit dedup, predictable output, and the two `let` blocks are easy to test independently.
